...
## D-046 — Make Product V2 Undo a permanent, idempotent forget operation

- **Status:** Accepted for the explicitly authorized final Product V2 Undo and operational-logging task; not a benchmark optimization or E006.
- **Context:** The integrated Product V2 runtime treated Undo as an append-only semantic retraction. The final product scope requires a mistaken capture to disappear from raw storage, processing, Memory, Attention, Ask, provenance, and attachment retrieval while preserving unrelated state and preventing an in-flight provider result from resurrecting it.
- **Decision:** Keep source evidence immutable during normal operation, but make explicit user Undo the narrow destructive exception. Under one SQLite write transaction, delete the selected source, processing row, source-linked derived/provenance rows, and unreferenced content-addressed blobs; scrub shared derived references; rebuild the remaining projections; and record only an event-ID/deletion-time tombstone so the identifier cannot be silently reused. Return an idempotent `already_deleted` result on repetition. Keep the existing `/api/v2/retract` and `retractCapture` names as compatibility surfaces while routing them to the permanent operation. Add a local line-oriented logger that emits lifecycle/status/count/timing data only, with bounded sanitization for identifiers and errors and no capture/question/provider payloads.
- **Consequences:** Pending, processing, processed, failed, attachment-only, shared-attachment, correction, Memory, Attention, Ask, provenance, idempotency, and late-result race paths are covered by deterministic regressions. A provider result that loses its processing lease after Undo is discarded without marking or recreating the deleted event. The visible 50-case Product V2 acceptance expectation is updated to permanent forgetting; the frozen V1 benchmark, official baseline/evaluator/calibration evidence, holdout material, and prior benchmark experiments are unchanged. Live subscription-provider smoke remains bounded and is reported separately from deterministic acceptance.
- **Revisit when:** A separately authorized retention/export, multi-user, or provider/storage scope changes the deletion boundary. Preserve normal source immutability, rebuildability for remaining state, explicit unknown semantics, approval boundaries, provider-token ownership, and holdout isolation.

## D-047 — Keep the final Product V2 dogfood repair conservative and bounded

- **Status:** Accepted for the explicitly authorized final Product V2 human-dogfood scope; the overall live gate remains PARTIAL/REVISE.
- **Context:** The final P0/P1 pass needed to reduce time to useful semantic Memory and Attention and make Attention, Memory, Ask, and operational logs visibly usable without reopening the settled architecture or frozen benchmark.
- **Decision:** Keep the compact versioned extraction instruction, bounded prior-memory context, supported low reasoning effort selected by the three-call ordinary-case comparison, conservative batch size two, and the UI/logging repairs. Preserve immediate capture, ordered projection, capture-time temporal anchoring, semantic truth, provenance, permanent Undo, and provider-free deterministic Ask. Do not add an invented fast/priority flag or tune wording after observing live outputs.
- **Consequences:** Deterministic focused coverage is clean and the two required viewport reviews pass. The fresh live smoke stayed within six captures and four Ask requests: all HTTP requests succeeded, Ask remained provider-free, and the relative due time was correct. The observed remaining burst interval (`129.562s`) does not establish material end-to-end latency improvement, and the launcher stream omitted final stop lines, so the overall gate is honestly PARTIAL/REVISE rather than PASS.
- **Revisit when:** A separately authorized bounded runtime experiment can measure provider startup/burst attribution and complete clean shutdown evidence. Preserve the frozen benchmark/evaluator/baseline and holdout boundaries.

## D-048 — Preserve human dogfood summaries with guarded values and relevance-ranked context

- **Status:** Accepted as a fix-first correction to the final Product V2 human-dogfood scope; the overall live gate remains PARTIAL/REVISE.
- **Context:** Review found that extraction context selected list tails rather than capture-relevant rows, default learned summaries could expose raw-like or credential-shaped values, UI coverage asserted source strings instead of executing behavior, and HTML manifest/icon cache references remained at `v7` while the rest of the shell was `v8`.
- **Decision:** Keep default human-readable derived summaries, but guard concept/key/value fields so private credential-like concepts, generic captures, nested payloads, opaque values, and raw-like echoes become metadata-only placeholders while benign scalar location/cost summaries remain useful. Rank bounded extraction rows by deterministic overlap with current capture terms, then sequence/captured time and canonical tie-breaks. Expose an opt-in inert Node hook for pure production UI helpers and disclosure-state behavior, and require every versioned shell asset reference to use `v8`.
- **Consequences:** The focused correction suite is `105/105`, including an older matching entity beyond the extraction limit, sensitive-value redaction, executable UI normalization/time/disclosure/Ask checks, and complete PWA version alignment. No provider or live call was made, measured live values were not altered, semantic/persistence/API boundaries remain unchanged, and the original PARTIAL/REVISE latency/shutdown judgment stands.
- **Revisit when:** A separately authorized live/runtime validation can measure burst attribution and clean shutdown. Do not change the default human-summary policy to machine-only logging or widen context beyond bounded deterministic selection.

## D-049 — Keep legacy and Product V2 runtime tuning on separate configuration boundaries

- **Status:** Accepted as a fix-first correction to the final Product V2 human-dogfood scope; the overall live gate remains PARTIAL/REVISE.
- **Context:** The preceding Product V2 tuning changed shared runtime defaults to low reasoning and batch size two. That could alter the frozen/legacy V1 Host behavior and could leave an existing Home on legacy high/ten settings without the intended Product V2 tuning.
- **Decision:** Restore the shared legacy defaults and validation to high reasoning, batch size ten, and the previously supported max/high/medium effort set. Persist independent `product_reasoning_effort` and `product_batch_size` fields with Product V2 defaults low and two; missing product fields in an existing config resolve to those Product V2 defaults in memory without changing its legacy fields. Legacy Host/engine/status and Product V2 lazy runtime, readiness discovery, queue utility, and direct runtime/provider defaults consume their respective boundaries. Keep the runtime config version and existing field names compatible.
- **Consequences:** Fresh Homes persist both boundaries, old Homes continue to load legacy values while Product V2 receives low/two, and low is accepted only by Product V2 discovery/provider validation. Deterministic boundary tests cover fresh and existing Homes, Host separation, direct defaults, Product V2 readiness, and queue utility argument wiring. No provider/live call, benchmark artifact, or semantic/persistence boundary changed; the prior first-use and burst latency/shutdown limitations remain the honest PARTIAL/REVISE gate.
- **Revisit when:** A separately authorized runtime configuration migration or provider capability scope changes the legacy/Product V2 ownership boundary. Preserve backwards-compatible config loading, immutable evidence, rebuildability, and frozen benchmark behavior.

## D-050 — Keep final Product V2 dogfood state generic, inspectable, and human-readable

- **Status:** Accepted for the explicitly authorized final Product V2 dogfood-fix scope; not a benchmark optimization or E006.
- **Context:** Final human dogfooding exposed duplicate Attention projections, terminal lifecycle residue, stale countdowns and capture explanations, history-first Memory, occurrence claims incorrectly competing as state, raw structured values in Ask, and missing bounded conversational context.
- **Decision:** Consolidate Attention at the projection boundary using conservative same-occurrence keys and related lifecycle links, retain completed/cancelled lifecycle status for inspection while exposing only open items to active UI and badge counts, and omit superseded timelines from the current projection. Mark explicit occurrence/event/transaction primitives as coexisting current rows and aggregate numeric values deterministically; leave ordinary state facts on the existing reconciliation path. Keep Ask thread context session-local and bounded to eight text messages, use prior assistant text only as a referent hint, and derive factual answers from Product V2 evidence. Render structured values, uncertainty, provenance, capture time, Memory hierarchy, countdowns, and loading state through human-readable defensive UI helpers with reduced-motion behavior.
- **Consequences:** One logical actionable occurrence produces one canonical Attention item with merged evidence; Done removes it from active UI and the badge while its lifecycle status remains inspectable. Current-first Memory groups history under its entity/topic, explicit occurrence totals remain provider-free, and Ask follow-ups can resolve referents without persisting chat context. The final visible acceptance is `50/50 PASS`, application coverage is `184/184`, evaluator coverage is `10/10`, and acceptance latency is PASS. No extraction/baseline prompt, provider configuration/performance policy, benchmark, evaluator, calibration, holdout, or V1 oracle/scoring material changed or was accessed; the only provider-facing text change is the runtime answer-boundary instruction that marks temporary thread context as non-evidence.
- **Revisit when:** A separately authorized provider, transport, unseen-validation, or post-freeze generalization scope changes these boundaries. Preserve immutable raw sources, rebuildable derived state, explicit unknown semantics, approval boundaries, provenance, provider-token ownership, and holdout isolation.

## D-051 — Make the final live Ask and Memory boundary current-first and occurrence-safe

- **Status:** Accepted for the explicitly authorized final live Ask + Memory + UI hotfix; not a benchmark optimization or E006.
- **Context:** The final live UX request identified two trust failures at the product boundary: a self-contained Ask question could inherit a stale thread topic, and ordinary event captures could be rendered as unknown, conflicting state, or duplicated history. The same review required natural primary answers, actionable clarification, current-first Memory disclosures, and mobile-safe Capture and Attention controls.
- **Decision:** Treat the current Ask question as authoritative. Use prior thread content only as a bounded referent hint for an explicitly referential follow-up; prefer the latest turn, never use prior assistant text as evidence, and derive public source references from selected current facts. Keep deterministic location, arithmetic, temporal, and occurrence answers provider-free, render provenance/retrieval details as secondary support, and constrain any provider semantic answer to supplied evidence. Detect occurrences generically across ordinary event verbs while guarding state qualifiers, preserve them as coexisting known event facts, and aggregate them deterministically. Make genuine unknown clarification an explicit `Clarify in Ask` navigation/prefill action with no auto-submit or persisted fact. Keep Memory current-first with closed `History · N` and `Occurrences · N` disclosures; give Attention Done a dedicated footer/action area and keep Capture controls within the approved mobile geometry.
- **Consequences:** Self-contained topic switches no longer reuse stale context; referential follow-ups retain the latest relevant topic; real occurrence facts remain known occurrence state and do not create false clarification or conflict; primary answers no longer expose retrieval diagnostics or raw timestamps; and clarification is actionable without mutating memory. The final focused suite is `39/39`, the application suite `196/196`, the evaluator `10/10`, the acceptance harness `7/7`, the root suite `213/213`, and integrated acceptance is `50/50 PASS` with all seven quality gates and latency PASS. The bounded synthetic live smoke used 4 captures and 3 asks, completed all processing on one attempt per event with zero failures/retries, and passed the required topic-switch and real-occurrence checks. No provider configuration, prompt/model/reasoning/batch policy, frozen benchmark, evaluator, holdout, calibration, or V1 oracle/scoring material changed or was accessed.
- **Revisit when:** A separately authorized provider, transport, unseen-validation, or post-freeze generalization scope changes the Ask evidence boundary, occurrence ontology, or UI contract. Preserve raw evidence, rebuildable derived state, explicit unknowns, approval boundaries, provider-token ownership, and holdout isolation.

## D-052 — Anchor relative-day semantics to capture-local calendar dates

- **Status:** Accepted for the explicitly authorized final relative-day temporal correctness hotfix; not a benchmark optimization or E006.
- **Context:** Final Mac human dogfooding after the Ask/Memory/UI hotfix exposed a deterministic occurrence-total response that rendered a capture on 2026-08-31 saying “yesterday” as Aug 29 instead of Aug 30. The trace was `mode=occurrence_totals`, `provider_used=false`; the temporal shape could contain a raw relative expression alongside an inconsistent provider-proposed absolute date.
- **Decision:** Normalize the bounded relative-day values `today`, `yesterday`, and `tomorrow`, plus `dzisiaj`, `wczoraj`, and `jutro`, from the persisted capture timestamp's local calendar date in its capture timezone. Raw relative expression evidence takes precedence over an inconsistent absolute proposal. Use local calendar arithmetic rather than arbitrary UTC 24-hour subtraction, preserve the capture timezone through occurrence projections, and compare occurrence dates in that timezone. Leave absolute timestamps, relative minutes/hours, weekdays, deadlines, reschedules, provider configuration, and the frozen benchmark/evaluator boundaries unchanged.
- **Consequences:** Temporal fields and occurrence Ask aggregation now resolve Aug 31 “yesterday” to Aug 30, including across a ZoneInfo DST transition, while the raw source and rebuildable derived-state boundaries remain intact. Deterministic focused coverage is `12/12`, the application suite is `200/200`, the root suite is `217/217`, and integrated Product V2 acceptance is `50/50 PASS` with no live provider. This is final correctness evidence, not a V1 benchmark, baseline, holdout, calibration, or E006 result.
- **Revisit when:** A separately authorized temporal contract or product-generalization scope changes relative-day vocabulary, capture timezone persistence, or occurrence presentation. Preserve immutable raw evidence, deterministic derived state, explicit unknowns, provenance, approval boundaries, provider-token ownership, and holdout isolation.

## D-053 — Make the final Product V2 READY Ask and lifecycle projections coherent

- **Status:** Accepted for the explicitly authorized final Product V2 coherence and demo-ready scope; not a benchmark optimization and not E006.
- **Context:** The final product review found that normal READY Ask answers could remain deterministic prose even when a usable AI provider was available, completed Attention items could disappear from inspectable history or fail to close their predecessor, document captures lacked useful identity, and the Attention Done footer and processing notice were visually misaligned.
- **Decision:** Keep retrieval, temporal normalization, arithmetic, occurrence aggregation, provenance, lifecycle validation, deduplication, and rebuildable projections deterministic. Pass bounded structured candidates plus deterministic derived results to the local subscription-first provider, require explicit candidate evidence selection, and use provider prose as the normal READY renderer. Mark provider failures, unavailable providers, invalid evidence, or unsafe/internal prose as an explicit degraded fallback. Persist terminal Attention lifecycle state for Memory/history while exposing only open items to active Attention and badge counts. Reconcile completion, payment, cancellation, rescheduling, and supersession through explicit lifecycle/entity/event links or strong unique entity matches; never through weak title-only matching. Preserve generic document identity fields and attachment provenance, and fix the UI at the CSS cascade/container boundary.
- **Consequences:** Simple retrieval, occurrence totals, current state, completed history, topic switches, and referential follow-ups use provider-rendered answers when bounded evidence exists; deterministic fallback remains visible and logged when it is necessary. Done is a user-marked completion, not proof of an external-world action. Documents can surface type/reference/issuer/service/amount/date/status when actually known, with unknowns preserved. The final coherence suite is `9/9`, the application suite is `216/216`, the evaluator suite is `10/10`, the acceptance harness is `7/7`, the root suite is `233/233`, and visible integrated acceptance is `50/50 PASS`. The bounded live demo used five synthetic captures and four Ask requests with provider-backed factual answers, restart persistence, lifecycle history, and 390×844/1280×900 visual checks. The frozen benchmark, official baseline/evaluator/calibration evidence, holdout boundary, and V1 oracle/scoring semantics were not changed or accessed.
- **Revisit when:** A separately authorized provider/schema, transport, performance, or post-freeze generalization scope changes the normal READY answer boundary or lifecycle model. Preserve immutable raw sources, rebuildability, explicit unknowns, approval boundaries, provenance, provider-token ownership, and holdout isolation. D-051 remains the historical record of the prior provider-free Ask policy; this decision supersedes it only for the final coherence scope.

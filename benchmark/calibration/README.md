...
**Status:** non-scored calibration only; Gate A is not frozen.
...
The calibration stream is independent of the proposed Life Inbox storylines
...
context limit, tokenizer, and provider configuration are pinned. The same
fixed baseline instruction and the same fixed query bundle must be used at
all four sizes. No prompt, tool policy, temperature, or answer format may be
changed in response to an individual calibration failure.
...
cannot replace a realistic primary benchmark. No final benchmark length is
approved by this calibration artifact alone.

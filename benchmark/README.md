...
evaluator-controlled holdout material. Gate A is approved. The primary
...
The frozen contract is [benchmark/dev/contract.json](dev/contract.json). It
defines stable scenario identifiers, immutable normalized captures, typed
assertions, known/inferred/unknown semantics, fixed checkpoint queries,
longitudinal scoring, DSCR, deterministic financial observations, duplicate and
correction semantics, evaluator boundaries, and the approval boundary. The
human-readable review artifact is [benchmark/dev/REVIEW.md](dev/REVIEW.md).
